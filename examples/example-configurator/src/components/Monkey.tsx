...
export default function Monkey() {
  ...
  const [layers, setLayers] = useState<LAYERS.Abstract[]>([])
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const l = json.layers.map((layer: SerializedLayer) => {
        // @ts-ignore
        const c = new LAYERS[layer.constructor]() as LAYERS.Abstract

        for (const key in layer.properties) {
          // @ts-ignore
          c[key] = layer.properties[key]
        }
        return c
      })
      ...
      <TransformControls ref={transformControls}>
        <group scale={0.1} rotation={[0, -Math.PI / 2, Math.PI / 4]}>
          <mesh matrixAutoUpdate geometry={nodes.Suzanne.geometry} rotation-y={Math.PI / 2} scale={30}>
            <DebugLayerMaterial {...materialProps} layers={layers} />
          </mesh>
        </group>
      </TransformControls>
